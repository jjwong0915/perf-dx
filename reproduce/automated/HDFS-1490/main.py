...
def main():
    ...
    # run checkpoint of secondary namenode
    try:
        subprocess.run([f"{HADOOP_HOME}/sbin/start-dfs.sh"])
        subprocess.run([f"{HADOOP_HOME}/sbin/hadoop-daemon.sh", "stop", "secondarynamenode"])
        #
        with contextlib.ExitStack() as stack:
            checkpoint = stack.enter_context(subprocess.Popen([
            ...
            pool.apply_async(collect_metric, ["secondarynamenode", checkpoint.pid])
            ...
            stack.enter_context(subprocess.Popen(
                ["tee", f"{parent}/log/secondarynamenode.log"],
                stdin=checkpoint.stdout,
                stdout=sys.stdout,
            ))
            # simulate namenode not sending image
