...
def main():
    ...
    try:
        ...
        #
        with contextlib.ExitStack() as stack:
            # HDFS get command does not produce log
            # so only server logs are collected
            pool = stack.enter_context(multiprocessing.Pool(4))
